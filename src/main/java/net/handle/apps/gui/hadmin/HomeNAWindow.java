...
public class HomeNAWindow extends JDialog implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if (src == okButton) {
            ...
            HelpPanel.show(tool, CommonDef.HELP_DIR, CommonDef.HELP_HOME_NA);
        }
    }
    ...
    /** Home the prefix at the specified server.  If we can't,
      return false. */
    private boolean homeNA() {
        try {
            ...
            GenericRequest homeNAReq = new GenericRequest(naHandle, AbstractMessage.OC_HOME_NA, authInfo);
            ...
            for (int i = 0; i < siteInfo.servers.length; i++) {
                response = tool.resolver.sendRequestToServer(homeNAReq, siteInfo, siteInfo.servers[i]);
                if (response.responseCode != AbstractMessage.RC_SUCCESS) {
                    throw new Exception(String.valueOf(response));
                }
            }

            GenericDialog.askQuestion("Success", "The prefix was successfully homed on\n" + "all servers within the given site.", GenericDialog.QUESTION_OK, this);
            return true;
        } catch (Exception e) {
            e.printStackTrace(System.err);
            GenericDialog.askQuestion("Error", "Error: " + e + "\nPlease try again", GenericDialog.QUESTION_OK, this);
        }
